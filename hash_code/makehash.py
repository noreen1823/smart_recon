...
def hashes(path):
    ...
    # Debugging: Print out base directory
    ...
    for file in os.listdir(base):
        ...
        if os.path.isfile(full_path):  # Check if it is a file
            print(f"Found file: {full_path}")  # Debugging: Print the found file path
            with open(full_path, 'rb') as pcap:
                str1 = pcap.read()
                ...
                hashlist.append(file)
                hashes.append(hashlist)
        else:
            print(f"Skipping non-file entry: {file}")

    out_dir = '/home/kali/Desktop/sensor-detection-paper/code/smart_reckon/hash_code/'
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, '1-minute-hashes-cleaned.csv')

    # Debugging: Print the output path
    print(f"Writing output to: {out_path}")

    # Write the hashes to an output file
